...
/*!50001 VIEW `student_result_view` AS select `s`.`name` AS `name`,`sub`.`subject_name` AS `subject_name`,`m`.`marks` AS `marks` from ((`students` `s` join `marks` `m` on((`s`.`student_id` = `m`.`student_id`))) join `subjects` `sub` on((`m`.`subject_id` = `sub`.`subject_id`))) */;
...
-- Dump completed on 2025-12-17 10:50:36
